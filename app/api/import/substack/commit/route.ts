...
interface CommitBody {
  ...
  /** Onboarding: import every importable post in the job at one global price. */
  applyToAll?: boolean;
...
export async function POST(request: Request) {
  try {
    ...
    let query = supabase.from("creator_import_candidates")
      ...
      .eq("creator_id", creatorId).eq("import_job_id", body.jobId).eq("status", "preview");
    if (!body.applyToAll) query = query.in("id", selections.map((item) => item.id));
