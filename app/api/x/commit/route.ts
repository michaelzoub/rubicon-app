...
/**
 * Bulk-publish a selected X writer's Articles through the same pricing handoff
 * as Substack/Artemis onboarding. Tweet (status) URLs are rebuilt from the
 * validated handle and ids instead of trusting client-provided URLs, then run
 * through the shared `importX` pipeline (which falls back to fxtwitter for the
 * full X Article body).
 */
export async function POST(request: Request) {
  try {
    ...
    if (!/^[A-Za-z0-9_]{1,15}$/.test(handle) || selections.length === 0) {
      return responseError(400, "invalid_selection", "Select at least one X article.");
    }
    if (new Set(selections.map((item) => item.id)).size !== selections.length) {
      return responseError(400, "invalid_selection", "Each X article can only be selected once.");
    }

    const accessMode = body.accessMode === "free" ? "free" : "paid";
    const imported = await mapWithConcurrency(selections, 4, async (selection) => {
      const sourceUrl = `https://x.com/${handle}/status/${selection.id}`;
      const result = await importFromUrl(sourceUrl);
      if (!result.body) {
        throw new ImportServerError(502, "article_parse_failed", `Could not read “${result.title || selection.id}” from X.`);
      }
    ...
    if (articleError) {
      console.error("X article insert failed", {
      ...
      if (articleError.code === "23514" && articleError.message.includes("articles_source_platform_check")) {
        ...
        if (!articleError) {
          console.warn("Published X articles against the legacy source-platform constraint; apply the latest Supabase migration.");
        }
      }
      if (articleError) {
        ...
        throw new ImportServerError(500, "article_create_failed", "Could not publish the X articles.");
      }
    }
    ...
    if (sectionRows.length > 0) {
      ...
      if (error) throw new ImportServerError(500, "section_create_failed", "Could not save the X article sections.");
    }
    ...
    if (revisionError) throw new ImportServerError(500, "revision_create_failed", "Could not save the X article revisions.");
    ...
    return responseError(500, "import_failed", "Could not publish the X articles.");
  }
}
